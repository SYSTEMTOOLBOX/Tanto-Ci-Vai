...
qr_local_pattern=r'<script src="\./tcv-qr-local\.js\?v=\d+"></script>'
...
qr_local_tag='<script src="./tcv-qr-local.js?v=1"></script>'
qr_auth_tag='<script src="./community-qr-auth-fix.js?v=4"></script>'
...
if re.search(qr_local_pattern,s):
    s=re.sub(qr_local_pattern,qr_local_tag,s,count=1)
else:
...
if re.search(qr_auth_pattern,s):
    ...
    s=s.replace(qr_local_tag,qr_local_tag+'\n'+qr_auth_tag,1)
