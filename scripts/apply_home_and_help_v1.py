...
original=s
marker='/* TCV_HOME_ADDRESS_HELP_V1 */'
if marker in s:
    print('Home/help patch already applied')
    raise SystemExit(0)

css=r'''
/* TCV_HOME_ADDRESS_HELP_V1 */
...
fn_anchor='function openNewRequest(step=1){'
if fn_anchor not in s:
    raise SystemExit('openNewRequest anchor not found')
fn_prefix="""function openNewRequest(step=1){
  if(step===2&&PROFILE?.home_city){
    const full=[PROFILE.home_street||'',PROFILE.home_civic||''].filter(Boolean).join(' ');
    HOME_POS={lat:+PROFILE.home_lat,lng:+PROFILE.home_lng,label:PROFILE.home_label||canonicalDeliveryLabel(full,PROFILE.home_city),city:PROFILE.home_city,street:full,cityText:PROFILE.home_city,streetText:full,civicText:PROFILE.home_civic||''};
    if(!Number.isFinite(HOME_POS.lat)||!Number.isFinite(HOME_POS.lng))HOME_POS=null
  }
"""
...
s=s.replace('<input id="nrCity" value="" placeholder="Es. Lauriano"','<input id="nrCity" value="${esc(PROFILE?.home_city||\'')}" placeholder="Es. Lauriano"',1)
s=s.replace('<input id="nrStreet" value="" placeholder="Es. Via Anselmina"','<input id="nrStreet" value="${esc(PROFILE?.home_street||\'')}" placeholder="Es. Via Anselmina"',1)
s=s.replace('<input id="nrCivic" value="" placeholder="Es. 14/A"','<input id="nrCivic" value="${esc(PROFILE?.home_civic||\'')}" placeholder="Es. 14/A"',1)
...
print('Saved home + community help patch applied')
