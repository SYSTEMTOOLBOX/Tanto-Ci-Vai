...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==='status'){
      ...
      if(!row?.authorization_id)return json({ok:true,status:'NONE',confirmed:false,linked:false});
      ...
      return json({ok:true,status,confirmed:status==='ACCEPTED',linked:status==='ACCEPTED'});
    }
    ...
    if(action==='create'){
      ...
      if(existing?.authorization_id&&existing.status==='ACCEPTED')return json({ok:true,status:'ACCEPTED',confirmed:true,linked:true});
      const idem=`tcv-satispay-link-${user.id}-${crypto.randomUUID()}`;
      const a=await sat('POST','/g_business/v1/pre_authorized_payment_tokens',{
        reason:'Collegamento Satispay per pagamenti Tanto Ci Vai',
      ...
      return json({ok:true,sandbox:true,status,confirmed:status==='ACCEPTED',linked:status==='ACCEPTED',authorization_id:a.id,redirect_url:a.redirect_url||null});
    }
