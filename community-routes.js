...
(function(){
  ...
  // Community anti-profit model. The passenger contribution is capped so the
  ...
  function ownTripsHtml(){
    ...
    return `<div class="tcv-mytrips">${mine.map(t=>`<article class="tcv-tripcard ${t.active?'active':''}"><div class="tcv-triphead"><strong>${t.active?'● GIRO ATTIVO':'○ GIRO DISATTIVATO'}</strong><span>${num(t.distance_km).toFixed(1)} km</span></div><h3>${safe(t.from_label)} → ${safe(t.to_label)}</h3><p>${scheduleText(t.schedule)}<br>👥 ${Number(t.seats||1)} posti · <b>${money(antiProfitContribution(num(t.distance_km),Number(t.seats||1)).perPerson)}</b> a persona · fino a €0,15/km · tetto €0,45/km auto</p><div class="tcv-tripactions"><button class="toggle" onclick="tcvToggleCommunityTrip('${t.id}',${t.active?'false':'true'})">${t.active?'DISATTIVA':'ATTIVA'}</button><button onclick="openTripSearch('${t.id}')">MODIFICA</button><button onclick="tcvDeleteCommunityTrip('${t.id}')">ELIMINA</button></div></article>`).join('')}</div>`
  }
  ...
  window.publishRideRequest=async function(){
    const when=document.getElementById('rideWhen')?.value||'',note=document.getElementById('rideNote')?.value.trim()||'',flex=Math.max(0,Math.min(180,Number(document.getElementById('rideFlex')?.value||30))),passengers=Math.max(1,Math.min(MAX_COMMUNITY_PASSENGERS,Number(document.getElementById('ridePassengers')?.value||1))),st=document.getElementById('rideStatus'),btn=document.getElementById('ridePublishBtn');
    ...
    try{
      ...
      const [a,b]=await Promise.all([resolvePlace('rideFromTown','rideFrom','from','ride'),resolvePlace('rideToTown','rideTo','to','ride')]),per=antiProfitContribution(rideDraft.route.distanceKm,passengers).perPerson;
