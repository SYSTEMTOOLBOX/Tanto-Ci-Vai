...
(function(){
  ...
  async function photon(q,limit=6){
    ...
    const r=await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(query+', Italia')}&limit=${limit}&lang=it`);
  ...
  window.tcvComputeTripPreview=async function(){
    ...
    try{const [a,b]=await Promise.all([resolveInput('tcvTripFrom','from','trip'),resolveInput('tcvTripTo','to','trip')]);const r=await routeDetails(a,b);draft.route=r;if(box)box.innerHTML=tcvPreviewHtml(r);if(st)st.textContent='✓ Percorso verificato. Controlla calendario e posti, poi salva.';return r}catch(e){if(st)st.textContent='Errore: '+e.message;throw e}
  };
  ...
  window.tcvUseGpsTripCommunity=function(){
    ...
    navigator.geolocation.getCurrentPosition(async p=>{const pt=await reversePoint(p.coords.latitude,p.coords.longitude);draft.from=pt;const el=document.getElementById('tcvTripFrom');if(el)el.value=pt.label;clearBox('tcvTripFromBox');if(st)st.textContent='✓ Partenza compilata automaticamente dal GPS.';tcvMaybeAutoPreview()},()=>{if(st)st.textContent='Consenti la posizione al telefono oppure scrivi l’indirizzo.'},{enableHighAccuracy:true,timeout:12000})
  };
  ...
  window.tcvComputeRidePreview=async function(){
    ...
    try{const [a,b]=await Promise.all([resolveInput('rideFrom','from','ride'),resolveInput('rideTo','to','ride')]);const r=await routeDetails(a,b);rideDraft.route=r;window.tcvRenderRidePrice();if(st)st.textContent='✓ Percorso verificato e contributo calcolato.';return r}catch(e){if(st)st.textContent='Errore: '+e.message;throw e}
  };
  ...
  window.tcvUseGpsRideCommunity=function(){
    ...
    navigator.geolocation.getCurrentPosition(async p=>{const pt=await reversePoint(p.coords.latitude,p.coords.longitude);rideDraft.from=pt;const el=document.getElementById('rideFrom');if(el)el.value=pt.label;clearBox('rideFromAuto');if(st)st.textContent='✓ Partenza compilata automaticamente dal GPS.';tcvMaybeRidePreview()},()=>{if(st)st.textContent='Consenti la posizione oppure scrivi l’indirizzo.'},{enableHighAccuracy:true,timeout:12000})
  };
  ...
  window.publishRideRequest=async function(){
    ...
    try{
      ...
      const [a,b]=await Promise.all([resolveInput('rideFrom','from','ride'),resolveInput('rideTo','to','ride')]),per=num(rideDraft.route.distanceKm)*RATE_PER_KM;
  ...
  function tripPopup(t){
    const per=num(t.distance_km)*RATE_PER_KM;return `<div class="tcv-map-popup"><b>🚗 GIRO COMMUNITY ATTIVO</b><div class="row"><strong>${safe(t.from_label)} → ${safe(t.to_label)}</strong></div><div class="row">👤 ${safe(t.driver_name||'Utente')} · 👥 ${Number(t.seats||1)} posti</div><div class="row">📏 ${num(t.distance_km).toFixed(1)} km · 💶 ${money(per)} / persona</div><div class="row">🗓️ ${scheduleText(t.schedule,true)}</div><div class="row">Servizio TCV: ${money(PLATFORM_FEE)} separato</div><button onclick="tcvOpenRideFromTrip('${t.id}')">RICHIEDI UN POSTO</button></div>`
  }
  ...
  window.initMap=function(){
    ...
    TRIPS.filter(t=>t.active).forEach(t=>{const coords=Array.isArray(t.route_coords)?t.route_coords:[];const latlngs=coords.map(c=>[Number(c[1]),Number(c[0])]).filter(c=>Number.isFinite(c[0])&&Number.isFinite(c[1]));if(latlngs.length<2)return;pts.push(latlngs[0],latlngs[latlngs.length-1]);const line=L.polyline(latlngs,{weight:6,opacity:.78}).addTo(MAP);line.bindPopup(tripPopup(t),{maxWidth:310});line.bindTooltip(`${safe(t.from_label)} → ${safe(t.to_label)} · ${money(num(t.distance_km)*RATE_PER_KM)}/persona`,{sticky:true})});
